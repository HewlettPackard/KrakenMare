...
dockerpull="--pull";
#DEFAULT ARGS
DEFAULT_INVENTORY_FILE_ROOT=${SETUP_HOME}/hosts;
...
usage () {
     ...
     echo "-d: to Deploy"
     echo "-i: to specify the inventory file (DEFAULT is ${DEFAULT_INVENTORY_FILE_ROOT}-`hostname`)"
...
if [ -z "$inventory_file" ] ; then
    this_host=`hostname`
    inventory_file=${DEFAULT_INVENTORY_FILE_ROOT}-${this_host}
    if [ ! -e "$inventory_file" ] ; then
	echo "" >&2;
	echo "***" >&2
	echo "building $inventory_file" >&2;
	echo "***" >&2
	sed "s/__HOST__/${this_host}/g" ${SETUP_HOME}/hosts-CI > ${inventory_file}
    fi
else
    if [ ! -f $inventory_file ] ; then
	echo "specified inventory file ${inventory_file} does not exits"
	exit 1
    else
	#inventory path must be absolute to be used for docker volumes
	cd `dirname $inventory_file` || exit 1
	path_inventory=`pwd` || exit 1
	file_inventory=`basename $inventory_file` || exit 1
	inventory_file=${path_inventory}/${file_inventory}
    fi
fi

#Registry vars
unset $registry;
registry=$(cat $inventory_file | sed -n -e '/\[registry\]/,$p' | grep -v "[\[,#,^$]" |sed  '/^$/d'| awk '{$1=$1};1' | head -1 | awk '{ print $1}' )
...
if [  "$ansible" == "1"  ]; then
     ...
     #docker volume mounting will create path when they do not exit, potentially damaging the CI/CD
     #so double check volumes exist prior to asking bind mounts

     pwd
     
     docker run --rm --volume ${SETUP_HOME}:/playbooks/ --volume $inventory_file:/etc/ansible/hosts --network=host ansible ansible-playbook /playbooks/kmu-client-playbook.yml --extra-vars "http_proxy=${http_proxy} https_proxy=${https_proxy} time_server=${time_server} dns_search="'"'"${dns_search}"'"'" dns_list="'"'"${dns_list}"'"'"" --forks 100 || { echo "warning: non zero exit code: $?" ; }
     ...
     docker run --rm --volume ${SETUP_HOME}:/playbooks/ --volume $inventory_file:/etc/ansible/hosts --network=host ansible ansible-playbook /playbooks/swarm_exit.yml --extra-vars "http_proxy=${http_proxy} https_proxy=${https_proxy} " --forks 100 || { echo "warning: non zero exit code: $?" ; }
     docker run --rm --volume ${SETUP_HOME}:/playbooks/ --volume $inventory_file:/etc/ansible/hosts --network=host ansible ansible-playbook /playbooks/swarm_init.yml --extra-vars "http_proxy=${http_proxy} https_proxy=${https_proxy} " --forks 100 || { echo "warning: non zero exit code: $?" ; }
