...
run_me zookeeper  timeout 10 /tmp/zookeeper-ready.sh || exit 1
...
echo -n "number of devices registered... @time = $now : "
curl -s http://elastic:9200/_cat/count/device-registration | cut -f 3 -d" "
