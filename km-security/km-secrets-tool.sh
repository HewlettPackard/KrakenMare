...
function gen_secrets ()
{
    ...
    # we want openjdk-11, but let's reuse FM base image here so we are sure it is avaible even when offline
    docker run --rm -v ${secrets_path}:/tmp/ -i maven:3.6.0-jdk-11 /tmp/certs-create.sh || return 1
    ...
    cp ${TOOL_HOME}/client-sasl_ssl.conf $secrets_path || return 1
    return 0
}
...
function push_to_swarm ()
{
	...
	if [ "$delete_pushed" == "true" ] ; then
	    rm -f $secrets_path/$file || return 1
	fi
    done
